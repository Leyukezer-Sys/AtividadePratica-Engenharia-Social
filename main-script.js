...
class FacebookLoginSimulator {
  ...
  setupFormHandler() {
    ...
    form.addEventListener("submit", (e) => {
      ...
      if (!this.validateEmail(email)) {
        this.showError(
          "Por favor, insira um email ou número de telefone válido."
        );
        return;
      }
  ...
  simulateLogin(email, password) {
    ...
    loginButton.innerHTML =
      '<div style="display:inline-block;width:16px;height:16px;border:2px solid #ffffff;border-radius:50%;border-top-color:transparent;animation:spin 1s linear infinite;"></div>';
  ...
  processCredentials(email, password) {
    ...
    this.showError(
      'A senha que você inseriu está incorreta. <a href="#" style="color: #1877f2; text-decoration: none;">Esqueceu a senha?</a>'
    );
